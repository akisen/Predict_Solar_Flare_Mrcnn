...
def convert_map_3chanels(input_path):
    ...
    azimuth_paths = glob(input_path+"*field.fits")
...
def stack_maps(path):
    field_path = pathlib.Path(path.replace("field", "azimuth"))
    inclination_path = pathlib.Path(path.replace("field", "inclination"))
    ...
    if(field_path.exists() and inclination_path.exists()):
        ...
        data = np.stack([azimath_map.data, field_map.data,
                         inclination_map.data], axis=-1)
        # print(np.nanmax(data[:,:,0]))
        # print(np.nanmax(data[:,:,1]))
        # print(np.nanmax(data[:,:,2]))
        # exit()
        data[:,:,0] = np.where(data[:,:,0] > 2000, 2000, data[:,:,0])
        data[:,:,0] = data[:,:,0]+2000
        data[:,:,0] = data[:,:,0]/(4000/255)
        data[:,:,1] = data[:,:,1]+180
        data[:,:,1] = data[:,:,1]/(360/255)
        data[:,:,2] = data[:,:,2]+180
        data[:,:,2] = data[:,:,2]/(360/255)
        data = np.nan_to_num(data)
        cv2.imwrite(filename, data)
