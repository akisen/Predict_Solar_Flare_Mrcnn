...
parser.add_argument("--span", default=24)
...
def main():
    ...
    flare_df_paths_dic = {path.split(
        "/")[-1][:-4]: path for path in flare_df_paths}
    ...
    for mask_path in tqdm(mask_paths, desc="{}-{}".format(start, end)):
        ...
        flare_df = pd.read_csv(flare_df_paths_dic[str(
            ar_num)], index_col=0, parse_dates=True)
        rec_datetime = dt.strptime(
            mask_map.meta["t_rec"][:-4], "%Y.%m.%d_%H:%M:%S")
        ...
        if (len(ar_polygon) == 1):
            if(len(ar_polygon[0]) != 2):
                ...
                add_flare_label(coord_df, flare_df, rec_datetime, span)
            else:
            ...
            for polygon in ar_polygon:
                if(len(polygon[0]) != 2):
                    ...
                    add_flare_label(coord_df, flare_df, rec_datetime, span)
    coord_df.to_pickle("{}/{}{}coord_df.pickle".format(args.output_path,
                                                       start.year, str(start.month).zfill(2)))


def padding_mask(mask_map):
    binarized_mask_map = np.where(
        (mask_map.data == 33) | (mask_map.data == 34), 1, 0)
    mask_center = np.array(
        [mask_map.reference_pixel[0].value, mask_map.reference_pixel[1].value])
    mask_ll = mask_center - \
        (mask_map.meta["naxis1"]//2, mask_map.meta["naxis2"]//2)
    mask_ur = mask_center + \
        (mask_map.meta["naxis1"]//2, mask_map.meta["naxis2"]//2)
    full_disk_coord = 4096
    pad_width = np.array([[full_disk_coord-mask_ur[1], mask_ll[1]],
                          [mask_ll[0], full_disk_coord-mask_ur[0]]])
    if(mask_ll[0] >= 0 and mask_ll[1] >= 0):
        padded_mask_map = np.pad(np.flipud(binarized_mask_map), np.array(
            pad_width, dtype="int"), "constant")
    else:
...
def rotate_map(mask_map, padded_mask_map):
    ...
    rotated_padded_mask_map = cv2.warpAffine(
        padded_mask_map.astype("int16"), trans, (width, height))
...
def polygonize_map(mask_map):
    ...
    ar_polygons = []
    ar_polygon_gen = shapes(binarized_mask_map.astype(
        "int16"), mask=None, connectivity=8)
    for ar_polygon in ar_polygon_gen:
        if(ar_polygon[0]["coordinates"][0][0] != (0.0, 0.0)):
            poly = [[coord[0]+ll_x, coord[1]+ll_y]
                    for coord in ar_polygon[0]["coordinates"][0]]
            poly_txt = ["{} {}".format(coord[0]+ll_x, coord[1]+ll_y)
                        for coord in ar_polygon[0]["coordinates"][0]]
...
def initialize_df(start, end):
    ...
    time_df = pd.DataFrame([[[], [], [], []] for i in range(len(
        time_index))], index=time_index, columns=["Polygon", "C_FLARE", "M_FLARE", "X_FLARE"])
    return time_df


def add_flare_label(coord_df, flare_df, rec_datetime, span):
    rec_datetime = rec_datetime
    with open("../logs/{}{}.txt".format(rec_datetime.year, str(rec_datetime.month).zfill(2)), mode="a") as f:
        if (is_flared(rec_datetime, span, flare_df, "C")):
            label = is_flared(rec_datetime, span, flare_df, "C")
            tqdm.write(label)
            coord_df.loc[rec_datetime]["C_FLARE"].append(label)
            f.write("{}:{}\n".format(rec_datetime, label))
        else:
            coord_df.loc[rec_datetime]["C_FLARE"].append(0)
        if (is_flared(rec_datetime, span, flare_df, "M")):
            label = is_flared(rec_datetime, span, flare_df, "M")
            tqdm.write(label)
            coord_df.loc[rec_datetime]["M_FLARE"].append(label)
            f.write("{}:{}\n".format(rec_datetime, label))
        else:
            coord_df.loc[rec_datetime]["M_FLARE"].append(0)
        if (is_flared(rec_datetime, span, flare_df, "X")):
            label = is_flared(rec_datetime, span, flare_df, "X")
            tqdm.write(label)
